...
int clone_child_function(void *arg) {
    ...
    ssize_t written = write(*fd, clone_line, strlen(clone_line));
    if (written < 0) {
        perror("Ошибка записи (clone-потомок)");
        exit(EXIT_FAILURE);
    }
    printf("[Clone-потомок] PID %d: Записано %zd байт в файл.\n", getpid(), written);
    fflush(stdout);

    // Принудительная запись данных на диск
    if (fsync(*fd) == -1) {
        perror("Ошибка fsync (clone-потомок)");
        exit(EXIT_FAILURE);
    }
...
void test_file_descriptors_clone(void) {
    ...
    // Запись родительского процесса перед clone
    const char *parent_line = "Родитель записывает строку перед clone\n";
    ssize_t written = write(fd, parent_line, strlen(parent_line));
    if (written < 0) {
        perror("Ошибка записи (родитель, перед clone)");
        close(fd);
        exit(EXIT_FAILURE);
    }
    printf("[Родитель] PID %d: Записано %zd байт в файл.\n", getpid(), written);
    fflush(stdout);
    ...
    // Создаем дочерний процесс через clone
    ...
    // Запись родительского процесса после clone
    const char *parent_after = "Родитель записывает строку после clone\n";
    written = write(fd, parent_after, strlen(parent_after));
    if (written < 0) {
    ...
    // Закрываем файл и освобождаем стек
    close(fd);
    free(stack);
...
int main(int argc, char *argv[]) {
    if (argc > 1 && strcmp(argv[1], "exec_child") == 0) {
        ...
        if (sigaction(SIGUSR1, NULL, &sa) == -1) {
            ...
            if (sa.sa_handler == SIG_DFL) {
                printf("[Exec-потомок] PID %d: Обработчик SIGUSR1 это значение по умолчанию.\n", getpid());
            } else {
                printf("[Exec-потомок] PID %d: Обработчик SIGUSR1 НЕ сброшен (неожиданно).\n", getpid());
            }
            fflush(stdout);
