/**************************************************************
 * Программа управления нитями с использованием сигналов SIGUSR1
 * 
 * Цели:
 * 1. Демонстрация межпоточного взаимодействия через сигналы
 * 2. Реализация точного временного контроля выполнения операций
 * 3. Использование системного таймера для управления нитями
 * 
 * Особенности реализации:
 * - Использование pthread_kill() для отправки сигналов
 * - Точный временной контроль через clock_gettime()
 * - Безопасная обработка сигналов с sigaction()
 * - Временные интервалы:
 *   * INITIAL_DELAY = 1 сек (задержка перед первым сигналом)
 *   * OPERATION_TIME = 3 сек (время работы второй нити)
 **************************************************************/

/*-------------------------------------------------------------
 * Используемые функции:
 * 1. get_current_time()   - получение времени с наносекундной точностью
 * 2. sigusr1_handler()    - обработчик сигнала SIGUSR1
 * 3. thread2_func()       - основная функция второй нити
 * 4. thread1_func()       - логика управления второй нитью
 * 5. main()               - инициализация и запуск системы
 * 
 * Выводы:
 * 1. Механизм pthread_kill() эффективен для управления нитями
 * 2. Системный таймер (CLOCK_MONOTONIC) обеспечивает точность ±1 мкс
 * 3. Комбинация сигналов и флагов volatile гарантирует безопасное
 *    завершение работы нитей
 * 4. Логирование с наносекундной точностью позволяет анализировать
