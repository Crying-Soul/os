 ...
 *  
 *  Цель:
 *      Измерить время выполнения и процент загрузки CPU для заданной команды.
 *
 *  Описание:
 *      Программа создает дочерний процесс, выполняет указанную команду и
 *      измеряет общее время выполнения, а также пользовательское и системное
 *      время процессора. Затем вычисляется процент загрузки CPU на основе
 *      этих измерений.
 *
 *  Использование:
 *      ./process_monitor <команда> [аргументы...]
 *      где <команда> - это исполняемый файл, который нужно запустить.
 *
 *  Пример:
 *      ./process_monitor ls -l /tmp
 *
 *  Вывод:
 *      <время_выполнения_в_секундах> <процент_загрузки_cpu>
 *
 *  Примечания:
 *      - Использует CLOCK_MONOTONIC_RAW для более точного измерения времени.
 *      - Перенаправляет stdout и stderr в /dev/null для уменьшения влияния
 *        вывода на производительность.
 *      - Использует wait4 для получения статистики CPU дочернего процесса.
 *
 ******************************************************************************/
