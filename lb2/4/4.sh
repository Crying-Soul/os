...
"""
Функциональность:
1. Комплексный анализ многопоточной программы:
   - Запуск и мониторинг целевой программы
   - Сбор диагностических данных:
     * Информация о потоках (ps -Lf)
     * Распределение нагрузки (ps -L -o)
     * Открытые файловые дескрипторы (lsof)
     * Карта памяти процесса (/proc/PID/maps)
   - Тестирование устойчивости через удаление TID

2. Особенности реализации:
   - Параллельный сбор логов программы и скрипта
   - Проверка зависимостей (pgrep, ps, lsof)
   - Умное завершение через kill последнего TID
   - Генерация итогового отчета (out.txt)

3. Технические аспекты:
   - Очистка лог-файлов перед запуском
   - Двойное логирование (консоль + файл)
   - Обработка ошибок поиска PID/TID
   - Фоновый запуск с контролем времени
"""
