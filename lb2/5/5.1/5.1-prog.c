...
void *thread_function(void *arg)
{
    ...
    while (iteration < 5)
    {
        ...
        printf("%-7s | PID: %-3d | TID: %-4ld | Nice: %-1d | Policy: %-12s\n",
               "Thread", getpid(), syscall(SYS_gettid), param.sched_priority, get_scheduling_policy(policy));
...
int main()
{
    ...
    while (iteration < 5)
    {
        ...
        // Выводим подробную информацию с фиксированной шириной столбцов
        printf("%-7s | PID: %-3d | TID: %-4ld | Nice: %-1d | Policy: %-12s\n",
               "Main", getpid(), syscall(SYS_gettid), param.sched_priority, get_scheduling_policy(policy));
