/**
 * 5.4.4-prog1.c / 5.4.4-prog2.c / 5.4.4-prog3.c
 * 
 * Функциональность:
 * 1. Демонстрация работы sched_yield() в SCHED_RR:
 *    - Установка Round Robin планирования с заданным приоритетом
 *    - Имитация CPU-нагрузки (цикл с инкрементом)
 *    - Явная передача управления через sched_yield()
 * 
 * 2. Ключевые особенности:
 *    - Использование SCHED_RR с одинаковым приоритетом (30)
 *    - 5 итераций работы с выводом состояния
 *    - Принудительный вызов sched_yield() после каждой итерации
 *    - Валидация входного приоритета (1-99)
 * 
 * 3. Технические детали:
 *    - Требует root-прав (sudo)
 *    - Привязка к ядру 0 через taskset в скрипте
 *    - Форматированный вывод информации о процессе
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < 5; i++){
        ...
        printf("Program 3: sched_yield_called\n");
        fflush(stdout);
