 ...
 *
 * Функциональность:
 * 1. Демонстрация управления приоритетами процессов в Linux:
 *    - Анализ и изменение nice-значений системных и пользовательских процессов
 *    - Сравнение возможностей изменения приоритетов для разных типов процессов
 *    - Демонстрация наследования приоритетов при fork()
 *
 * 2. Ключевые операции:
 *    - Получение текущего приоритета (getpriority)
 *    - Попытки изменения приоритета:
 *      * Для системного процесса (обычно PID 1)
 *      * Для пользовательского родительского процесса
 *      * Для дочернего процесса
 *    - Использование разных методов изменения:
 *      * setpriority() - установка точного значения
 *      * nice() - относительное изменение
 *
 * 3. Особенности реализации:
 *    - Обработка ошибок привилегий (требуются права root для некоторых операций)
 *    - Наглядное сравнение возможностей изменения приоритетов
 *    - Чистый вывод с форматированием для удобства анализа
 */
#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
