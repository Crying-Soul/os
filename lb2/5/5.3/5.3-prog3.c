/**
 * 5.3-prog1.c / 5.3-prog2.c / 5.3-prog3.c
 *
 * Функциональность:
 * 1. Демонстрация реального планирования FIFO:
 *    - Установка политики SCHED_FIFO с заданным приоритетом
 *    - Выполнение CPU-intensive нагрузки (цикл с инкрементом)
 *    - Периодический вывод информации о процессе
 *
 * 2. Ключевые операции:
 *    - Установка политики через sched_setscheduler()
 *    - Получение текущих параметров (sched_getscheduler, sched_getparam)
 *    - Валидация входного приоритета (с проверкой диапазона)
 *    - Форматированный вывод состояния процесса
 *
 * 3. Особенности:
 *    - Требует прав root для установки SCHED_FIFO
 *    - Привязка к ядру 0 через taskset в скриптах
 *    - Идентичная реализация в prog1/prog2/prog3 для чистоты эксперимента
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
