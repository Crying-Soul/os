 ...
 *
 * Функциональность:
 * 1. Исчерпывающая демонстрация работы всех функций семейства exec():
 *    - execl()  - выполнение с указанием полного пути (список аргументов)
 *    - execlp() - поиск в PATH (список аргументов) 
 *    - execle() - с пользовательским окружением (список аргументов)
 *    - execv()  - с массивом аргументов
 *    - execvp() - поиск в PATH + массив аргументов
 *    - execve() - полный контроль (аргументы + окружение)
 *
 * 2. Системные взаимодействия:
 *    - Создание процессов через fork() для каждого теста
 *    - Корректное ожидание завершения через wait()
 *    - Перехват и обработка ошибок выполнения
 *    - Управление переменными окружения
 *
 * 3. Диагностические возможности:
 *    - Четкое логирование каждого этапа
 *    - Вывод PID выполняющихся процессов
 *    - Контроль состояния через fflush()
