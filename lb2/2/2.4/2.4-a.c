 ...
 * 
 * Функциональность:
 * 1. Демонстрация жизненного цикла родительского и дочернего процессов:
 *    - Создание процесса через fork()
 *    - Параллельное выполнение кода родителя и потомка
 *    - Ожидание завершения потомка через wait()
 * 
 * 2. Мониторинг процессов:
 *    - Трехэтапный вывод состояния системы (до/во время/после выполнения)
 *    - Использование ps -H f для отображения иерархии
 *    - Анализ статуса завершения потомка (WIFEXITED/WEXITSTATUS)
 * 
 * 3. Особенности реализации:
 *    - Гарантированный сброс буферов ввода-вывода (fflush)
 *    - Четкое разделение кода родителя и потомка
 *    - Визуальное логирование каждого этапа выполнения
 */
#include <stdio.h>
#include <stdlib.h>
#include <sys/wait.h>
