...
static void long_computation(const char* process_name, int* var) {
    for (int i = 0; i < 100000000; i++) {
        if (i % 10000000 == 0) {
            printf("%-24s | PID = %-6d | PPID = %-6d | var = %-4d | адрес = %-14p | Итерация = %-10d\n", 
...
int main(void) {
    ...
    printf("%-24s | PID = %-6d | var = %-4d | адрес = %-14p\n", 
    ...
    if (pid < 0) {
        ...
        // Дочерний процесс
        printf("%-24s | PID = %-6d | PPID = %-6d | var = %-4d | адрес = %-14p | (до изменения)\n",
               "Дочерний процесс", getpid(), getppid(), shared_var, (void*)&shared_var);
        fflush(stdout);

        shared_var += 50;  // Изменяем копию переменной в дочернем процессе
        printf("%-24s | PID = %-6d | PPID = %-6d | var = %-4d | адрес = %-14p | (после изменения)\n", 
               "Дочерний процесс", getpid(), getppid(), shared_var, (void*)&shared_var);
        fflush(stdout);

        long_computation("Дочерний процесс", &shared_var);

        printf("%-24s | PID = %-6d | Завершение работы\n", "Дочерний процесс", getpid());
        fflush(stdout);
        _exit(EXIT_SUCCESS);
    } else {
        // Родительский процесс
        printf("%-24s | PID = %-6d | PPID = %-6d | var = %-4d | адрес = %-14p | (после fork)\n", 
    ...
    printf("%-21s | PID = %-6d | Завершение программы\n", "Конец", getpid());
    fflush(stdout);
    return EXIT_SUCCESS;
}
