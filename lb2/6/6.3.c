...
void test_signal_inheritance_fork(void)
{
    ...
    // Флаг SA_RESTART позволяет автоматически перезапустить прерванные системные вызовы
...
// Здесь дочерний процесс после fork() вызывает execl(), чтобы перезапустить программу
// с аргументом "exec", что позволяет проверить, что обработчик сигнала сбрасывается
// до значения по умолчанию (в обычном случае SIGUSR1 завершит процесс).
void test_signal_inheritance_exec(char *prog_name)
{
    ...
    if (pid == 0)
    { // Дочерний процесс: выполняем exec()
        sleep(1);
...
{
    // Если программа запущена с аргументом "exec", это означает, что мы попали в exec()-ветку.
