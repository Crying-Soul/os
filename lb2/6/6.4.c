...
int clone_func(void *arg) {
  ...
  // Получаем и выводим текущее смещение файла, демонстрируя, что оно
  // разделяется с родителем
...
void test_clone_inheritance(void) {
  ...
  // Открываем файл без O_APPEND, чтобы можно было проследить за изменением
  // смещения
  ...
  // Флаги clone: SIGCHLD (для уведомления о завершении) и CLONE_FILES
  // (разделение файловых дескрипторов)
  ...
  printf("Завершён тест clone наследования. Проверьте содержимое файла "
         "'out.txt'.\n\n");
  fflush(stdout);
}
