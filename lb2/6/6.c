...
/* Наследование маски сигналов: родитель блокирует SIGUSR2, потомок проверяет наследование блока */
void test_signal_mask_inheritance(void) {
    printf("=== Тест 6.5: Наследование маски сигналов ===\n");
    fflush(stdout);

    sigset_t block_set, old_set, curr_set;
    ...
    printf("Родитель (PID %d): Блокирован SIGUSR2\n", getpid());
    fflush(stdout);
    ...
    if (pid == 0) {
        if (sigprocmask(SIG_SETMASK, NULL, &curr_set) < 0) {
            perror("sigprocmask (child)");
            exit(EXIT_FAILURE);
        }
        if (sigismember(&curr_set, SIGUSR2))
            printf("Потомок (PID %d): SIGUSR2 заблокирован\n", getpid());
        else
            printf("Потомок (PID %d): SIGUSR2 не заблокирован\n", getpid());
        fflush(stdout);
        exit(EXIT_SUCCESS);
    } else {
        wait(NULL);
        /* Восстанавливаем оригинальную маску сигналов */
        if (sigprocmask(SIG_SETMASK, &old_set, NULL) < 0) {
            perror("sigprocmask restore");
        }
    }
    
    printf("Завершён тест наследования маски сигналов.\n\n");
    fflush(stdout);
}

/* Наследование лимитов ресурсов: демонстрация RLIMIT_NOFILE */
void test_rlimit_inheritance(void) {
    printf("=== Тест 6.6: Наследование лимитов ресурсов (RLIMIT_NOFILE) ===\n");
    fflush(stdout);
    ...
    printf("Родитель (PID %d): RLIMIT_NOFILE: cur = %ld, max = %ld\n", getpid(), rl.rlim_cur, rl.rlim_max);
    fflush(stdout);
    ...
    if (pid == 0) {
        ...
        if (getrlimit(RLIMIT_NOFILE, &child_rl) < 0) {
            perror("getrlimit (child)");
            exit(EXIT_FAILURE);
        }
        printf("Потомок (PID %d): RLIMIT_NOFILE: cur = %ld, max = %ld\n", getpid(), child_rl.rlim_cur, child_rl.rlim_max);
        fflush(stdout);
        exit(EXIT_SUCCESS);
    } else {
        wait(NULL);
    ...
    printf("Завершён тест наследования лимитов ресурсов.\n\n");
    fflush(stdout);
}

/* Наследование переменных окружения: проверка значения и независимое изменение в потомке */
void test_env_inheritance(void) {
    printf("=== Тест 6.7: Наследование переменных окружения ===\n");
    fflush(stdout);

    if (putenv("MY_VAR=parent_value") != 0) {
        perror("putenv");
        exit(EXIT_FAILURE);
    }
    printf("Родитель (PID %d): MY_VAR=%s\n", getpid(), getenv("MY_VAR"));
    fflush(stdout);
    ...
    if (pid == 0) {
        printf("Потомок (PID %d): MY_VAR=%s (до изменения)\n", getpid(), getenv("MY_VAR"));
        setenv("MY_VAR", "child_value", 1);
        printf("Потомок (PID %d): MY_VAR=%s (после изменения)\n", getpid(), getenv("MY_VAR"));
        fflush(stdout);
        exit(EXIT_SUCCESS);
    } else {
        wait(NULL);
        printf("Родитель (PID %d): MY_VAR=%s (после изменения в потомке)\n", getpid(), getenv("MY_VAR"));
        fflush(stdout);
    }
    
    printf("Завершён тест наследования переменных окружения.\n\n");
    fflush(stdout);
}

/* Наследование текущей рабочей директории: изменение в потомке не влияет на родителя */
void test_working_directory_inheritance(void) {
    printf("=== Тест 6.8: Наследование текущей рабочей директории ===\n");
    fflush(stdout);
    ...
    printf("Родитель (PID %d): Текущая директория: %s\n", getpid(), cwd);
    fflush(stdout);
    ...
    if (pid == 0) {
        ...
        printf("Потомок (PID %d): Изменил директорию на: %s\n", getpid(), cwd);
        fflush(stdout);
        exit(EXIT_SUCCESS);
    } else {
        wait(NULL);
        ...
        printf("Родитель (PID %d): Текущая директория после fork: %s\n", getpid(), cwd);
        fflush(stdout);
    }
    
    printf("Завершён тест наследования текущей рабочей директории.\n\n");
    fflush(stdout);
}
